...
std::string SimpleSerial::ReadSerialPort(double reply_wait_time, string syntax_type) {
	...
	while ((chrono::steady_clock::now() - start_time) < std::chrono::duration<double>(reply_wait_time)) {

		if (status_.cbInQue > 0) {

			if (ReadFile(io_handler_, inc_msg, 1, &bytes_read, NULL)) {

				if (syntax_type._Equal("none")) {
					return(std::string(inc_msg));
				}
